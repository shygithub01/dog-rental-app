  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result.user) {
        ...
        const savedRole = udoc.exists() ? (udoc.data()?.role ?? null) : null;

        if (!savedRole) {
          // First time: create with chosen role
          await createUserProfileWithRole(result.user, selectedRole);
          return;
        }

        // If saved role differs from chosen, ask for confirmation
        if (savedRole !== selectedRole) {
          setRoleMismatch({ saved: savedRole, chosen: selectedRole, user: result.user });
          return;
        }

        // Roles match, proceed (refresh profile state)
        setUserProfile({ ...udoc.data(), id: result.user.uid });
      }
    } catch (error) {
